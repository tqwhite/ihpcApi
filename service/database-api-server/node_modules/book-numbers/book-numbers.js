...
const purchaseCodesGen = require('./lib/purchase-codes');
...
var moduleFunction = function(args) {
	...
	const purchaseCodes = purchaseCodesGen({ config: this.config.purchaseCodes });
	...
	const recordTransaction = (registerArgs, callback) => {
		const localCallback = (err, result) => {
			...
			const transactionIsGood = !result.length; //ie, this exact transaction (book number) has not previously been created
	...
	const generateSpecialBookNumber = (purchaseCode, callback) => {
		 
		const taskList = [];

		taskList.push((args, next) => {
			const localCallback = (err, bookNumber) => {
				next(err, bookNumber);
			};

			purchaseCodes.makeBookNumber(purchaseCode, localCallback);
		});

		taskList.push((args, next) => {
			const { bookNumber } = args;
			const localCallback = (err, bookNumberObject) => {
				next(err, bookNumberObject);
			};

			const item = new BookNumber({
				number: bookNumber,
				category: 'purchaseCode',
				role: 'nurse',
		...
		asynchronousPipe(taskList, initialData, (err, bookNumberObject) => {
			if (err) {
				callback({
					errmsg: err
				});
	...
	const checkBookNumberAvailability = (registerArgs, callback) => {
		...
		if (purchaseCodes.isPurchaseCode(registerArgs.bookNumber)) {
			generateSpecialBookNumber(registerArgs.bookNumber, localCallback);
			return;
		}
		...
		initDb.initializeDb((err, result) => {
			if (err) {
				next({
					code: '500',
					errorText:
						'Database access problem (bookNumber.reinitialize()) ' + err,
					err: err
				});
			} else {
				res.json({
