...
var moduleFunction = function(args) {
		...
		taskList.push((asyncNext) => {
			getAccessList({userRefId:req.tokenJwtDecoded.userRefId}, (err, result) => {
				if (err) {
					asyncNext({
						code: '500',
						message: 'Database access problem (student.get())',
						err: err
					});
				} else {
	...
	this.router.put(route, (req, res, next) => {
		...
		putItem(req.params[0], req.body, (err, result) => {
			if (err) {
				next({
					code: '500',
					message: 'Database access problem (student.put())',
					err: err
				});
			} else {
				res.json({
		...
		taskList.push((asyncNext) => {
			postItem(req.body, (err, result) => {
				if (err) {
					asyncNext({
						code: '500',
						message: 'Database access problem (student.post())',
						err: err
					});
				} else {
					res.json({
	...
	this.router.delete(route, (req, res, next) => {
		...
		deleteItem(req.params[0], (err, result) => {
			if (err) {
				next({
					code: '500',
					message: 'Database access problem (student.delete())',
					err: err
				});
			} else {
				res.json({
	...
	this.router.get(route, (req, res, next) => {
		initDb.initializeDb((err, result) => {
			if (err) {
				next({
					code: '500',
					message: 'Database access problem (student.reinitialize()) ' + err,
					err: err
				});
			} else {
				res.json({
	...
	const UserStudentsAccessModel = mongoose.model('userStudentsAccessSchema', userStudentsAccessSchema);
