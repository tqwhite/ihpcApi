...
var moduleFunction = function(args) {
	...
	const transferOneStudent = (
		...
		taskList.push((args, next) => {
			...
			args.senderAccessDoc.studentRefIdList = tmp; //UserStudentsAccessModel.studentRefIdList
			
			args.receiverAccessDoc.studentRefIdList=args.receiverAccessDoc.studentRefIdList.concat([args.studentRefId]); //UserStudentsAccessModel.studentRefIdList
			
			args.senderAccessDoc.transferSendingRefIdList.includes(transferRefId) ||
				(args.senderAccessDoc.transferSendingRefIdList=args.senderAccessDoc.transferSendingRefIdList.concat([transferRefId]));
				
			args.receiverAccessDoc.transferReceivingRefIdList.includes(
				transferRefId
			) ||
				(args.receiverAccessDoc.transferReceivingRefIdList=args.receiverAccessDoc.transferReceivingRefIdList.concat([transferRefId]));
						...
						const guardianList = [
							{
								...
								zip: item.gOneZip,
								emailAddress: item.gOneEmailAdr,
							...
							},
							{
								...
								zip: item.gTwoZip,
								emailAddress: item.gTwoEmailAdr,
								phoneMain: item.gTwoPhoneMain,
								phoneAlt: item.gTwoPhoneAlt
							}
						];

						outItem.guardianList = guardianList.filter(removeeEmptyRecords);
					}
						...
						const hcProviderList = [
							{
								...
								state:'',
								zip:'',
								emailAddress:item.hcOneEmailAdr,
								phoneMain:item.hcOnePhone,
								phoneFax: ''
							},
							{
								description: '',
								...
								state:'',
								zip:'',
								emailAddress:item.hcTwoEmailAdr,
								phoneMain:item.hcTwoPhone,
								phoneFax: ''
							}
			
						];
						outItem.hcProviderList = hcProviderList.filter(removeeEmptyRecords);
					}
					return outItem;
				});
				if (err) {
