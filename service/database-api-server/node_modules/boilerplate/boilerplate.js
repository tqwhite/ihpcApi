...
var moduleFunction = function(args) {
	...
	this.post=function(inData, callback){
		


		var user = new Boilerplate(inData);
		user.save(callback);
	...
	this.delete=function(id, callback){
		...
		Boilerplate.remove({_id:id}, callback);
	...
	this.router.delete(route, (req, res, next) => {
		...
		this.delete(req.params[0], (err, result) => {
			if (err) {
					next({
						code: '500',
	...
	this.router.get(route, (req, res, next) => {
		initializeDb((err, result) => {
			if (err) {
					next({
						code: '500',
						message: 'Database access problem (boilerplate.reinitialize())',
						err:err
					});
			} else {
				res.json({token:this.permissionMaster.updateToken(req), data:result});
	...
	const debugDataVersion = 129;

	let debugDataInx = 0;
	let userData = {};

	const addBoilerplate = (callback) => {
		...
		user.save((err, result) => {
			...
			if (debugDataInx < debugDataList.length) {
				setTimeout(addBoilerplate, 100, callback);
			}
			else{
				if (callback){
					callback(err, result);
	...
	const initializeDb=(callback)=>{
			...
			Boilerplate.remove({}, (err, result) => {
				...
				Boilerplate.count((err, result) => {
					console.log('INITIALIZING: TEST BOILERPLATE');
					addBoilerplate(callback);
				});
			});
