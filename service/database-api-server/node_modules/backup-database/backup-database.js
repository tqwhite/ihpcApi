...
var moduleFunction = function(args) {
	...
	this.initializeBackups = (callback) => {
		var textSched = later.parse.text('at 01:30am every day');
	//	var textSched = later.parse.text('every 3 secs');

		var timer2 = later.setInterval(backupMongo, textSched);
		callback();
	}
	...
	const backupMongo = () => {
		...
		if (!process.env.srapiBackupsPath) {
			console.log("NO DATABASE BACKUP. Environment var srapiBackupsPath is not defined for " + this.config.system.name + ".");
			return;
		}
		...
		exec(cmd, (err, stdout, stderr) => {
			if (!err) {
				console.log("Successful Mongo Backup: backupPath=" + backupPath + " for " + this.config.system.name);
			} else {
				const m1 = "FAILED Mongo Backup: backupPath=" + backupPath + " for " + this.config.system.name + "=====START"
				const m2 = "\n\n---err=" + err;
				const m3 = "\n\n---stderr=" + stderr;
				...
				const errPath = process.env.srapiBackupsPath + "mongoDb/mongoDb_" + dateString + "_FAILED_BACKUP.txt";
	...
	const purgeOlder = () => {
		...
		if (excess) {
			...
			for (var i = 0; i < excess; i++) {
				...
				console.log("PURGING backup set " + filePath + " for " + this.config.system.name);
			}
		}
...
shutting down ${__dirname}`);
	...
	if (process.env.srapiBackupsPath) {
		...
		this.mongoBackupCount = this.config.database.backupCount?this.config.database.backupCount:4;
	}
