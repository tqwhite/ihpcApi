	...
	this.router[method](route, (req, res, next) => {
		...
		if (userName) {
			this.apiManager.getApi('databaseApiServer.bookNumbers.users.getList')({
				...
				if (!users.length || users[0].pwhash != qtools.passwordHash(password, users[0].salt)) {
				...
				this.apiManager.getApi('databaseApiServer.bookNumbers.users.setLastLogin')(
					...
					()=>{
						...
						const claims={
							...
							lastDayInSubscription:users[0].lastDayInSubscription?users[0].lastDayInSubscription:calcDefaultSubscription(users[0].createdAt),
							databaseName:this.config.database.displayName,
							_id:users[0]._id
						}
						res.json({token:this.permissionMaster.getToken(role, claims), data:users});
					}
				)
