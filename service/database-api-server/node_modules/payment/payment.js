...
var moduleFunction = function(args) {
	...
	const processPayment = (args, callback) => {
		const { poInfo, ccInfo, userInfo, usePurchaseOrder, productInfo } = args;
		authorizeApp.processCreditCardTransaction(args, callback);
	};
	...
	const constructTransactionSummaryString=args=>{
	 return ('authCode:' +
					qtools.getSurePath(args, 'paymentResult.response.transactionResponse.authCode', '!authCode')) +
				'|' +
				('transId:' + qtools.getSurePath(args, 'paymentResult.response.transactionResponse.transId', '!transId')) +
				'|' +
				('code:' +
					qtools.getSurePath(args, 'paymentResult.response.transactionResponse.messages.message[0].code', '!code')) +
				'|' +
				('description:' +
					qtools.getSurePath(args, 'paymentResult.response.transactionResponse.messages.message[0].description', '!description')) +
				'|' +
				('prodCode:' + qtools.getSurePath(args, 'product.code', '!prodCode'));
	}
	...
	const postItem = (inData, callback) => {
		...
		taskList.push((args, next) => {
			...
			this.apiManager.getApi('databaseApiServer.bookNumbers.recordTransaction')(
				{
					number:
						qtools.getSurePath(args, 'paymentResult.CreateTransactionRequest.transactionRequest.order.invoiceNumber'),
					summaryString: constructTransactionSummaryString(args),
					months: args.product.months,
					role: args.product.role,
					category: qtools.getSurePath(args, 'paymentResult.response.paymentProcess'),
