...
// 				}
	...
	const processPo = (args, callback) => {
		...
		if (errorList) {
			callback(errorList[0].errorText);
			return;
		}
	...
	const processPayment = (args, callback) => {
		...
		const errorMessage = checkCcInfo(ccInfo);

		if (errorMessage) {
			callback(errorMessage);
			return;
		}
		authorizeApp.processTransaction(args, localCallback);
	};
	...
	const makeDictionary = inObj => {
		const flattenedList = qtools.flattenObject(inObj, {includeIntermediates:true});

		const paths = Object.keys(flattenedList);

		const outArray = [];
		paths.forEach(path => {
			if (flattenedList[path] instanceof Date) {

				outArray.push({
					pattern: path,
					replacement: flattenedList[path].toLocaleString()
				});

			} 
			else if (!['function', 'object'].includes(typeof flattenedList[path])) {
				outArray.push({
					pattern: path,
					replacement: flattenedList[path]
				});
			}
		});
	...
	const sendEmailMessages = (args, callback) => {
		const taskList = [];
		let template = 'renewalReceiptCreditCard';

		if (args.usePurchaseOrder == 'true') {
						taskList.push(next => {
							this.apiManager.getApi('mailTool.messages.addMessage')(
								{
									clientRefId:
										'payment.js/sendEmailMessages/renewalInvoicePurchaseOrder',
									emailAddress: args.user.emailAddress,
									immediateFlag: true,
									templateName: 'renewalInvoicePurchaseOrderAuthorizer',
									dictionary: makeDictionary(args)
								},
								(err, result) => {
									next(err, {
										clientRefId:
			...
			template = 'renewalInvoicePurchaseOrderPurchaser';
		}

				taskList.push(next => {
					this.apiManager.getApi('mailTool.messages.addMessage')(
						{
							clientRefId: `payment.js/sendEmailMessages/renewalNotificationAcumen`,
							emailAddress: args.user.emailAddress,
							immediateFlag: true,
							templateName: 'renewalNotificationAcumen',
							dictionary: makeDictionary(args)
						},
						(err, result) => {
							next(err, {
		...
		taskList.push(next => {
			this.apiManager.getApi('mailTool.messages.addMessage')(
				{
					clientRefId: `payment.js/sendEmailMessages/${template}`,
					emailAddress: args.user.emailAddress,
					immediateFlag: true,
		...
		async.series(taskList, (err, result) => {
			callback('', result);
		});
	};
	...
	const postItem = (inData, callback) => {
		...
		taskList.push((args, next) => {
			...
			if ((+product.price)!=(+args.productInfo.price)){
				next(`product pricing is incorrect ${args.productInfo.price} does not match ${product.price}`);
				return;
			}
		...
		taskList.push((args, next) => {
			const localCallback = (err, user) => {
				args.renewedUser = user[0].toObject();
				next(err, args);
			};
			getUser(args, localCallback);
		});
		...
		taskList.push((args, next) => {
			const localCallback = (err, messageSending) => {
				args.messageSending = messageSending;
				next(err, args);
			};
			sendEmailMessages(args, localCallback);
		});
	...
	if (false && qtools.getSurePath(this, 'config.system.environment') == 'development') {
		//not needed any more (false && ...) I think. Throw it out later if I'm right. tqii 11/26/18
