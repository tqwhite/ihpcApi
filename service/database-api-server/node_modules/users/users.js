	...
	const registerUser = (inData, callback) => {
		...
		this.bookNumbersModel.checkBookNumberAvailability(inData, (err, bookId) => {
			...
			postItem(inData, (err, result) => {
				...
				sendEmail(result, 'confirmEmail', (err, result)=>{
...
// 			console.log("\n=-=============   =============================  =========================\n");
	...
	const resendEmail = (inData, callback) => {
		...
		const localCallback=(err, result)=>{
			if (!err){
				...
				callback('', {message:`sent email to ${result.emailAddress}`});
				}
				
				sendEmail(result, 'confirmEmail', internalCallback);
				
				}
				else{
				callback({errorText:'ERROR! You accessed the resendEmail API'});
				}
		};
	...
	this.permissionMaster.addRoute(method, route, 'all');
	this.router[method](route, (req, res, next) => { 
		resendEmail(req.body, (err, result) => {
			if (err) {
				next({
					code: '500',
					errorObject: err,
