...
var moduleFunction = function(args) {
	...
	const incrementExpiration = (user, months, callback) => {
		...
		const workingDate=(lastDayInSubscription>today)?lastDayInSubscription:today;
	...
	const updateSubscription = (user, updateSubscriptionToken, callback) => {
		...
		if (!this.config.storeData[tokenBits[0]]) {
			callback(
				{
					errMsg: `The purchase token is invalid. There is no such store as ${tokenBits[0]}.`
				},
				''
			);
			return;
		...
		if (!tokenDetails) {
			callback(
				{
					errMsg: 'The purchase token is an invalid format. Perhaps some characters are missing.'
				},
				''
			);
			return;
		}
		console.log("tokenDetails="+tokenDetails.join(',')+" [users.js.updateSubscription()]");
		...
		return (err, userRec) => {
			if (!err) {
				const internalCallback = (err, dontCareAboutEmailResult) => {
					...
					if (emailBits) {
						obscureEmail = `${emailBits[1]}...${emailBits[2]}@${emailBits[3]}...${emailBits[4]}.${emailBits[5]}`;
					} else {
	...
	const encipher = this.apiManager.getApi(
		'webInit.dispatch.utilityServer.encipher'
	);
