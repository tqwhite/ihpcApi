...
var moduleFunction = function(args) {
	...
	const registerUser = (inData, callback) => {
		...
		this.apiManager.getApi('databaseApiServer.bookNumbers.checkBookNumberAvailability')(inData, (err, bookId) => {
			...
			postItem(inData, (err, result) => {
				...
				resendConfirmationEmail(result, 'confirmEmail', (err, result2) => {
	...
	const resendConfirmationEmail = (inData, template, callback) => {
		const userRefId = inData.refId;
		...
		const localCallback = (err, result) => {
			if (!err) {
				...
				callback({
					errorText: 'ERROR! You accessed the resendConfirmationEmail API'
				});
			}
		};
	...
	this.permissionMaster.addRoute(method, route, 'all');
	this.router[method](route, (req, res, next) => {
		resendConfirmationEmail(req.body, 'confirmEmail', (err, result) => {
			if (err) {
				next({
					code: '500',
					errorObject: err,
