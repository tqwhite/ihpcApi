...
var moduleFunction = function(args) {
	...
	// ==============================================================================
	// GET USER COMPLETE, FOREIGN KEYS FILLED OUT
	...
	const getUserComplete = (userData, callback) => {
		...
		taskList.push((args, next) => {
			const localCallback = (err, district) => {
				next(err, { ...args, district });
			};

			if (!args.user || !args.user.districtRefId) {
				next('', args); //no user found is not an error but requires no subesquent retrieving
				return;
			}

			const district = this.apiManager.getApi(
				'databaseApiServer.bookNumbers.users.session.boilerplate.payment.transfer.students.districts.getDistrictByRefId'
			)({
				refId: args.user.districtRefId,
				callback: localCallback
			});
		});
		...
		asynchronousPipe(taskList, initialData, (err, finalResult) => {
			...
			// On 12/15/23, I added ".toObject()" to the first assignment of the user object below. Before
			// that, none of the subordinate elements that were added had worked. The comments,
			// "to send these..." are a result of my not figuring that .toObject() was required to
			// add properties to the object.
			//
			// I am leaving the (I suspect) extraneous transfer property assignments in place because
			// I don't want to test changes to it. However, i am sure that the only thing that
			// is used in the UI is the new district property.

			const user = finalResult.user.toObject(); //new ".toObject()" change
			...
			user.district = finalResult.district
				? finalResult.district[0].toObject()
				: undefined;
	...
	// ==============================================================================
	// GET USER SIMPLE, JUST RECORD DATA
	...
	const getUserIfValid = (potentialUsername, callback) => {
		...
		getUserData(
			{ username: new RegExp(`^${potentialUsername}$`, 'i') },
			'',
			localCallback
		);
	};
