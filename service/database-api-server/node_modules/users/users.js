...
var moduleFunction = function(args) {
	...
	const figureOutError=(err) =>{
		...
		if (err.code=='11000'){
			...
			errorObj={errorSource:'users', errorText:errorText};
			return errorObj;
		}
	
	
		errorObj={errorSource:'users', errorText:err.errmsg};
	...
	let route = new RegExp('(?:user$|user/([A-Za-z0-9]*)$)');
	let method='get';
	this.permissionMaster.addRoute(route, 'admin editor nurse');
	...
	this.router[method](route, (req, res, next) => {
		...
		putItem(req.tokenJwtDecoded.role, req.params[0], req.body, (err, result) => {
			if (err) {
				next({
					code: '500',
					errorObject: figureOutError(err),
					err: err
				});
			} else {
				res.json({
	...
	method='post';
	this.permissionMaster.addRoute(route, 'all', method);
	this.router[method](route, (req, res, next) => {
