...
var moduleFunction = function(args) {
	...
	const registerUser = (inData, callback) => {
		this.apiManager.getApi('databaseApiServer.bookNumbers.checkBookNumberAvailability')(inData, (err, bookId) => {
			if (err) {
				callback(err);
				return;
			}
			postItem(inData, (err, result) => {
				...
				sendEmailToUser(result, 'confirmEmail', (err, result2) => {
					this.apiManager.getApi('databaseApiServer.bookNumbers.updateBookNumber')(bookId, result.refId, callback);
				});
			})
		});
		return;
	...
	const changePassword = (inData, callback) => {
		...
		const threshold=this.config.system.forgotEmailExpireMinutes?this.config.system.forgotEmailExpireMinutes:20;
		...
		const localCallback = (err, result) => {
			if (!err) {
				User.update({
					...
					pwhash:qtools.passwordHash(inData.newPassword)
				}, (err2, result2) => {
					callback('', {
						message: `Thank You. The password for ${result.username} is now changed.`,
		...
		return (err, result) => {
			if (!err) {
				const internalCallback = () => {
					const emailBits=result.emailAddress.match(/^(\w)\w*(\w)@(\w)\w*(\w)\.(\w\w\w)$/);

					const obscureEmail=`${emailBits[1]}...${emailBits[2]}@${emailBits[3]}...${emailBits[4]}.${emailBits[5]}`;

					callback('', {
						message: `sent email to ${result.emailAddress}`,
						obscureEmail:obscureEmail
					});
				}
				sendEmail(result, template, internalCallback);
			} else {
				callback({
	...
	const sendFeedback = (userData, template, callback) => {
		const messageData=Object.assign({}, {feedbackMessage:userData.feedbackMessage}, userData.user)
		...
		if (userData.forgotPasswordInfo) {
			query = {
				$or: [
					...
					{
						booknumber: userData.forgotPasswordInfo
					}
				]
			};
		} else {
	...
	this.apiManager.registerApi('encryptForForgotPassword', encryptForForgotPassword);
