...
var moduleFunction = function(args) {
	...
	this.permissionMaster.addRoute(route, 'nurse editor admin');
	this.router.get(route, (req, res, next) => {
		initDb.initializeDb((err, result) => {
			if (err) {
				next({
					code: '500',
					message: 'Database access problem (plan.reinitialize()) ' + err,
					err: err
				});
			} else {
				res.json({
	...
	this.router.get(route, (req, res, next) => {
		...
		taskList.push((asyncNext) => {
			if (!req.query.studentRefId) {
				asyncNext({
					code: '500',
					message: 'No studentRefId specified (plan.get())'
				});
				return;
			}
			getAccessList({studentRefId:req.query.studentRefId}, (err, result) => {
				if (err) {
					asyncNext({
						code: '500',
						message: 'Database access problem (plan.get())',
						err: err
					});
				} else {
	...
	this.router.put(route, (req, res, next) => {
		...
		putItem(req.params[0], req.body, (err, result) => {
			if (err) {
				next({
					code: '500',
					message: 'Database access problem (plan.put())',
					err: err
				});
			} else {
				res.json({
		...
		taskList.push((asyncNext) => {
			postItem(req.body, (err, result) => {
				if (err) {
					asyncNext({
						code: '500',
						message: 'Database access problem (plan.post())',
						err: err
					});
				} else {
					res.json({
	...
	this.router.delete(route, (req, res, next) => {
		...
		deleteItem(req.params[0], (err, result) => {
			if (err) {
				next({
					code: '500',
					message: 'Database access problem (plan.delete())',
					err: err
				});
			} else {
				res.json({
	...
	const planPlansAccessSchema = mongoose.Schema({
		refId: String,
		studentRefId: String,
	...
	const StudentPlansAccessModel = mongoose.model('planPlansAccessSchema', planPlansAccessSchema);

	const initDbGen = require('plans-test-db');
	const initDb = new initDbGen({
		oneToManyAccessModel: StudentPlansAccessModel,
		oneToManyAccessModelName: 'planPlansAccess',
		model: Plan,
		dbName: 'plan',
		config: this.config
	});
