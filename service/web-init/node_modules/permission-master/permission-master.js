...
console.dir({"decoded":decoded});
	...
	this.getToken=(user)=>{
		...
		const claims={ role:user.role, expiration:expTime, userRefId:user.refId };
console.dir({"claims":claims});
	...
	if (!req.tokenJwtDecoded && req.publicRole){
		...
		const expTime=Date.now()+this.config.system.sessionExpireMinutes*60000;
		const claims={ role:req.tokenJwtDecoded.role, expiration:expTime, userRefId:req.tokenJwtDecoded.userRefId };
		return {
			claims:claims,
			jwt:jwt.sign(claims, this.config.system.secret)
		}
