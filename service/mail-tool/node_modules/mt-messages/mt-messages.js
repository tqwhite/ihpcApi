...
var moduleFunction = function(args) {
	...
	const processAsyncTransformations = (asyncronousTransformations, replaceObject, callback) => {
		...
		const taskList = Object.keys(asyncronousTransformations).map(
			fieldName => {
	...
	const applyTemplate = (item, mailConfig, callback) => {
		...
		taskList.push((args, next) => {
			...
			processAsyncTransformations(mailConfig.asyncronousTransformations, args.replaceObject, localCallback);
		});
	...
	const startSystem = () => {
		...
		if (qtools.getSurePath(this.config, 'mtMessages.suppressAutoEmailEntirely', false).toString()=='true') {
			qtools.logWarn(`NO AUTOEMAIL: config.mtMessages.suppressAutoEmailEntirely is true`);
			return;
		}
		...
		if (qtools.getSurePath(this.config, 'mtMessages.useDebugSchedule', false).toString()=='true') {
