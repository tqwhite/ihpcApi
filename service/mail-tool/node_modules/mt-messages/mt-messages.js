...
var moduleFunction = function(args) {
	...
	const fixUpAddresses=(config)=>{
		...
		if (config.ccAddresses){
			outObj.ccAddresses=qtools.convertNumericObjectToArray(config.ccAddresses);
		}
		else{
			outObj.ccAddresses=[];
		}
		if (config.bccAddresses){
			outObj.bccAddresses=qtools.convertNumericObjectToArray(config.bccAddresses);
		}
		else{
			outObj.bccAddresses=[];
		}
...
qtools.dump({"addresses":addresses});
this.config.mtMessages.returnAddress
