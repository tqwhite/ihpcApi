...
var moduleFunction = function(args) {
	...
	const toDictionary = user => {
		return Object.keys(user).map(key => {
			let value = user[key];
			if (user && user[key] && typeof user[key].toUTCString != 'undefined') {
	...
	const batchStartNotification = (templateDirectoryPath, next) => {
		qtools.logDetail(`starting generate/queue autoEmail batch`);
	...
	const buildTemplateList = (filePathList, next) => {
		const buildParameterBundle = (filePath, next) => {
			...
			const parameterBundle = {
				...
				timing: templateDetails.controls.timing,
				inactiveFlag: templateDetails.controls.inactiveFlag
			};
		...
		const taskList = parameterBundleList.map(parameterBundle => {
			return localNext => {
				...
				emailTasks.push((parameterBundle, next) => {
					const localCallback = (err, expiringUserList) => {
						if (err) {
							next(err);
							return;
						}
						qtools.logDetail(
							`queuing email '${parameterBundle.templateName}' for ${expiringUserList.length} users`
						);
						const mailSendingPackage = {
							expiringUserList: expiringUserList,
					...
					if (
						...
						this.apiManager.getApi('mailTool.messages.collector.getExpiringUsers')(
							parameterBundle,
							localCallback
						);
					}
				}); //get expiringUserList for parameterBundle

				emailTasks.push((mailSendingPackage, next) => {
					const messageList = mailSendingPackage.expiringUserList.map(user => {
						return userNext => {
						

						const emailAddress = user.emailAddressSecondary
							? `${user.emailAddress},${user.emailAddressSecondary}`
							: user.emailAddress;

							qtools.logDetail(
								`queuing ${emailAddress} for ${parameterBundle.templateName}`
							);
	...
	const executeBatch = () => {
		...
		asynchronousPipe(taskList, templateDirectoryPath, (err, result) => {
			if (err) {
				qtools.logError(`executeBatch() says, ${err.errorText}`);
			}
			qtools.logDetail('finished queuing autoEmail batch');
		});
		return;
	};
