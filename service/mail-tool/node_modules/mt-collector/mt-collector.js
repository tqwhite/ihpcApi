...
var moduleFunction = function(args) {
		...
		this.apiManager.getApi('databaseApiServer.bookNumbers.users.getList')(
			...
			(err, users) => {
				...
				users
					...
					.forEach(user => {
						console.log(
							`${user.username} / ${user.emailAddress} / ${user.daysSinceSubscribed} ${user.daysLeftInSubscription}`
						);
					});
				console.log('\n\n=========================\n\n');
			}
	...
	const generateOffsetQuery = (propertyName, offset) => {
		...
		const laterDate = midnight(workingDate).setDate(earlierDate.getDate()+1);//midnight the day after
	...
	const validateForExact=timing=>{
		...
		if (start > today || end < today) {
			return `no email being sent. ${today.toLocaleDateString('en-US')} is outside ${start.toLocaleDateString('en-US')}/${end.toLocaleDateString('en-US')} period (measured at midnight)`;
		}
			}
	...
	const exactDaysUntilExpire = {
		query: (timing) => {
		const today = midnight('today');
		if (timing.sendToThoseWhoMissedWindow && timing.startDate && midnight(timing.startDate).valueOf()==midnight('today').valueOf()){
	...
	const getUserList = (parameterBundle, callback) => {
		...
		this.apiManager.getApi(
			...
			if (timing.sendToThoseWhoMissedWindow){
				qtools.logMilestone(`found 'sendToThoseWhoMissedWindow', got ${users.length} catch up users`);
			}
